...
app.post('/home', upload.array('image[]', 10), async (req, res) => { //up to ten images otherwise error
  try {
    ...
    if (!item_name || !description || !pricing) {
      return res.status(400).send("Missing required fields", { message: 'Missing required fields', error:true });
    }
    ...
    res.status(500).send("Error creating listing or uploading image", { message: 'Error creating listing', error:true });
  }
});
...
app.post('/edit-listing', upload.array('image[]', 10), async (req, res) => {
  try {
    ...
    if (!listing || listing.user_id !== user.id) {
      return res.status(403).send("Unauthorized to edit this listing.", { message: 'Unauthorized to edit this listing.', error:true });
    }
    ...
    res.status(500).send("Error updating listing or uploading image", { message: 'Error updating listing', error:true });
  }
});
